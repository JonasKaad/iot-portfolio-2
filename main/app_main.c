...
void send_response(char *payload, int payloadSize)
{
    if (client == NULL)
    {
        ESP_LOGE(TAG, "Tried to send a response while client is null :/");
        return;
    }
    esp_mqtt5_client_set_publish_property(client, &publish_property);
    int msgid = esp_mqtt_client_publish(client, CONFIG_MQTT_RESPONSE_TOPIC, payload, payloadSize, 0, 0);

    ESP_LOGI(TAG, "Sent response with id: %d", msgid);
}
...
static void mqtt5_app_start(void)
{
    esp_mqtt5_connection_property_config_t connect_property = {
        ...
        // .response_topic = CONFIG_MQTT_RESPONSE_TOPIC,
        .correlation_data = "123456",
        .correlation_data_len = 6,
    };
    ...
    esp_mqtt_client_config_t mqtt5_cfg = {
        ...
        // .session.last_will.topic = CONFIG_MQTT_COMMAND_TOPIC,
